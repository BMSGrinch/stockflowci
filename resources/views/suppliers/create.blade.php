...
@section('page-header')
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-semibold text-slate-900">Ajouter un fournisseur</h1>
            <p class="text-sm text-slate-500 mt-1">
                Renseignez les informations de votre nouveau partenaire.
            </p>
        </div>
    </div>
@endsection
...
@section('content')
    <div class="rounded-2xl bg-white shadow-sm border border-slate-200 p-6 max-w-2xl">
        <form id="supplier-form" method="POST" action="{{ route('suppliers.store') }}" class="space-y-6">
            @csrf
