...
@section('page-header')
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-semibold text-slate-900">Ajouter un produit</h1>
            <p class="text-sm text-slate-500 mt-1">
                Renseignez les informations du nouveau produit à ajouter au stock.
            </p>
        </div>
    </div>
@endsection
        ...
        <form id="product-form" method="POST" action="{{ route('products.store') }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative">
                        <input type="number" step="0.01" min="0" id="purchase_price" name="purchase_price" value="{{ old('purchase_price') }}"
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative">
                        <input type="number" step="0.01" min="0" id="selling_price" name="selling_price" value="{{ old('selling_price') }}"
                               class="block w-full rounded-xl border border-slate-200 bg-slate-50/60 px-3 py-2 pr-12 text-sm text-slate-900 placeholder-slate-400 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none">
                        <span class="pointer-events-none absolute inset-y-0 right-3 flex items-center text-[11px] text-slate-500">
                            FCFA
                        </span>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" min="0" id="quantity" name="quantity" value="{{ old('quantity', 0) }}"
                           class="block w-full rounded-xl border border-slate-200 bg-slate-50/60 px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none">
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" min="0" id="alert_threshold" name="alert_threshold" value="{{ old('alert_threshold', 0) }}"
                           class="block w-full rounded-xl border border-slate-200 bg-slate-50/60 px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none">
                </div>
            </div>
            ...
            <div class="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50/60 px-4 py-3">
                ...
                <label class="inline-flex items-center gap-2 cursor-pointer select-none">
                    ...
                    <input type="checkbox" name="is_active" value="1" @checked(old('is_active', 1)) class="peer sr-only">
